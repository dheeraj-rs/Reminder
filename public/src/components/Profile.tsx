...
const Profile: React.FC = () => {
    ...
    const handleUpload = useCallback(async () => {
        ...
        try {
            ...
            await axios.post(`${API_BASE_URL}profileImage/${userId}`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            }).then(() => { setShowUserDetails(false), setSelectedFile(null) }
            )
    ...
    return (
        <section className=" max-w-full p-5 xl:p-14 text-[#47425b] md:min-w-[calc(100vw-75vw)] md:order-3 ">
            <div className="flex gap-3 items-center justify-between relative">
                <div>
                    <p className="font-semibold text-2xl tracking-wide md:text-base lg:text-2xl pointer-events-none">{userData.username}</p>
                    <p className="text-lg font-bold text-[#f5bd6c] md:text-xs lg:text-base cursor-pointer select-none" onClick={() => setShowUserDetails(!showUserDetails)}>My settings</p>
                        ...
                        <ul className="user-details bg-[#e7f0fa] absolute top-20 left-0 min-w-full px-10 h-60 rounded-md p-3 z-50 shadow-md">
                            <li className='flex flex-col items-center mb-4'>
                                <div className="relative w-16 h-16 rounded-full">
                                    <img src={(userData.userimage === "") ? useradd : userData.userimage} alt='img' className='w-full h-full rounded-full' />
                                    <span className='w-full h-full absolute top-3 left-0 text-center text-4xl opacity-40'>+</span>
                                    <input
                                        type="file"
                                        autoComplete="off"
                                ...
                                <label htmlFor="image-upload" className="mt-2 cursor-pointer text-blue-500" onClick={handleUpload}>{(selectedFile === null) ? 'Edit' : 'Apply'}</label>
                            </li>
                            <li className="text-center text-xl font-semibold mb-2">{userData.username}</li>
                            <li className="text-center text-sm text-gray-600 mb-2">{userData.email}</li>
                            <li className="text-center">
                                <button className="text-blue-500 cursor-pointer hover:underline" onClick={() => logout()}>Logout</button>
                                <button className="text-red-500 ml-2 cursor-pointer hover:underline" onClick={() => setShowUserDetails(false)}>Close</button>
                            </li>
                        </ul>
                    )}
            ...
            <div className="w-full shadowsAll rounded-xl mt-5 2xl:mt-28 p-5 bg-[#f9fbfd] flex flex-col items-center order-3 sm:order-3  ">
                ...
                <span className="w-full flex items-center justify-center gap-4 pb-5 pl-3 md:gap-2 lg:gap-5">
                    ...
                    <div className="w-full flex items-center gap-3 overflow-x-scroll">
                        ...
                        />
                        <span className='flex gap-1'><button onClick={prevSongHandler}>
                            <span className="material-symbols-outlined text-gray-600 hover:text-gray-800">fast_rewind</span>
                        </button>
                            <h1 className="text-xl w-20 overflow-y-auto font-bold md:text-base ">{songs[currentSongIndex].title}</h1>
                            <button onClick={nextSongHandler}>
                                <span className="material-symbols-outlined text-gray-600 hover:text-gray-800">fast_forward</span>
                            </button>
                        </span>
                    </div>
                </span>
            ...
            <div className="rounded-xl h-max mt-5 p-5 pb-0 bg-[#f9fbfd] shadowsAll">
                ...
                <div className="flex items-end gap-5 mt-3 md:gap-0 lg:gap-5 ">
                    <img src={useradd} alt="" className="sm:w-28 h-24 md:w-20 mix-blend-darken" />
