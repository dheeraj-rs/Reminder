...
const Today: React.FC = () => {
    ...
    const handleTogglePinned = useCallback(async (id: any) => {
        try {
            const updatedPinnedStatus = !listData.find((task) => task._id === id)?.pinned;
    ...
    const filteredData = useMemo(() => {
        return listData
            .filter((task) => new Date(task.date).toLocaleDateString() === selectDate.toLocaleDateString())
            .sort((a, b) => {
            ...
            <main className="h-[calc(100vh-24vh)] rounded-xl flex flex-col gap-3 overflow-y-scroll p-5 relative ">
                {filteredData.map((task) => (
                    <div
                        ...
                        key={task._id}
                        onDoubleClick={() => setShowOption(true)} // Show options on double click
                        onClick={() => setShowOption(false)}>
                        <div
                            className={`flex items-center p-2 justify-center cursor-pointer shadowsAll ${task.pinned ? 'bg-[#f1f1f1] text-[#f5bd6c]' : ''
                                } w-6 h-6 rounded-full text-lg font-semibold md:w-5 md:h-5 lg:w-6 lg:h-6`}>
                            <span
                                className={`material-symbols-outlined  ${task.pinned ? 'text-[#f5bd6c]' : ''
                                    } text-lg`} onClick={() => handleTogglePinned(task._id)}>
                                push_pin
                            </span>
                        </div>

                        <div
                            className={`flex-grow p-3 min-w-0 rounded-lg border overflow-x-scroll ${task.pinned ? 'border-[#f5bd6c] shadow-lg' : 'border-[#edeff3]'
                                }`}
                            ...
                            <div className="flex gap-4 w-full items-center  ">
                                ...
                                <h1 className="font-medium w-max md:text-lg " >{task.title?.slice(0, 20)}</h1>
                                ...
                                <div
                                    className={`flex items-center gap-2 cursor-pointer text-gray-400 hover:text-gray-600  transition-colors ${showOption ? 'block' : 'hidden'
                                        }`}>
                                    <span
                            ...
                            <div className="pl-[60px] text-xs">
                                {task.details?.map((detail: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | null | undefined, index: number) => (
                                    <li className={detail ? (index < 1 ? 'list-none' : 'list-disc') : ''} key={index}>
                                        {detail || ''}
                                    </li>
                                ))}
                            </div>
